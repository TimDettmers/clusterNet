...
void run_miniMNIST_test(ClusterNet gpus)
{
	...
	/*
	Matrix *w1_dist = scalarMul(gpus.rand(784,1000),0.4*sqrt(6.0f/(784.0+1000.0)));
	Matrix *w2_dist = scalarMul(gpus.rand(1000,10),0.4*sqrt(6.0f/(10.0+1000.0)));
	Matrix *m1_dist = zeros(784,1000);
	Matrix *m2_dist = zeros(1000,10);
	Matrix *ms1_dist = zeros(784,1000);
	Matrix *ms2_dist = zeros(1000,10);
	Matrix *grad_w1_ms_dist = zeros(784,1000);
	Matrix *grad_w2_ms_dist = zeros(1000,10);
	Matrix *grad_w2_dist = empty(1000,10);
	Matrix *grad_w1_dist = empty(784,1000);
	float cv_error = 0.0f;
	float train_error = 0.0f;
